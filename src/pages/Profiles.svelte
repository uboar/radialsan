<script lang="ts">
    ...
    try {
      ...
      alert(
        $t("profiles.importSummary", {
          profiles: pkg.profiles?.length || 0,
  ...
  function handleUpdateRule(index: number, updates: Partial<MatchRule>) {
    editRules = editRules.map((rule, ruleIndex) =>
  ...
  function getRuleCandidateValues(field: MatchRule["field"]): string[] {
    const values = windowCandidates
      .map((candidate) =>
        field === "processName" ? candidate.processName : candidate.windowTitle,
      )
  ...
  function handleModifierToggle(pieKey: PieKey, modifier: ModifierName) {
    ...
    handleUpdatePieKey(pieKey.id, {
      hotkey: buildHotkeyString(toggleModifier(modifiers, modifier), key),
    });
  }
  ...
  function handleKeyChange(pieKey: PieKey, key: string) {
    ...
    handleUpdatePieKey(pieKey.id, {
      hotkey: buildHotkeyString(modifiers, key),
    });
  }
  ...
  function getMatchFieldLabel(field: MatchRule["field"]): string {
    return $t(
      field === "processName" ? "profiles.processName" : "profiles.windowTitle",
    );
  }
            ...
            <div class="space-y-3">
              <div>
                <label
                  ...
                  for="profile-name">{$t("profiles.profileName")}</label
                >
                <input
                  id="profile-name"
                  type="text"
              ...
              <div>
                ...
                <div class="space-y-2">
                  {#each editPieKeys as pieKey (pieKey.id)}
                    ...
                    <div
                      class="flex min-w-0 flex-col gap-2 rounded-lg border border-theme-border bg-theme-bg-tertiary p-3"
                    >
                      <div class="flex min-w-0 items-center gap-2">
                        ...
                        >
                          <span
                            class="mr-1 shrink-0 text-xs text-theme-text-secondary"
                            >{$t("profiles.hotkeyModifiers")}:</span
                          >
                      ...
                      <div class="flex flex-wrap items-center gap-2">
                        <div class="flex min-w-0 items-center gap-1.5">
                          <span
                            class="shrink-0 text-xs text-theme-text-secondary"
                            >{$t("profiles.hotkeyKey")}:</span
                          >
                          <select
                            value={parsed.key}
                            onchange={(event) =>
                        ...
                        >
                          {recordingPieKeyId === pieKey.id
                            ? $t("profiles.recording")
                            : $t("profiles.recordKey")}
                        </button>
              ...
              {:else}
                <div class="flex flex-wrap gap-1.5">
                  {#each profile.pieKeys as pieKey (pieKey.id)}
                    {@const menuName =
                      $settingsStore.settings.menus.find(
                        (menu) => menu.id === pieKey.menuId,
                      )?.name ?? pieKey.menuId}
                    <span
                    ...
                    >
                      ...
                      <span class="min-w-0 truncate text-theme-text-primary"
                        >{menuName}</span
                      >
                    </span>
                  {/each}
