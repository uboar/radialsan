...
pub fn execute_lua_file(path: &Path) -> Result<(), LuaEngineError> {
    let script = std::fs::read_to_string(path).map_err(|e| LuaEngineError::Io(e))?;
...
fn register_api(lua: &Lua) -> Result<(), LuaEngineError> {
    ...
    let open_url = lua
        .create_function(|_, url: String| {
            open::that(&url).map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
            Ok(())
        })
        .map_err(LuaEngineError::Lua)?;
    ...
    let open_file = lua
        .create_function(|_, path: String| {
            open::that(&path).map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
            Ok(())
        })
        .map_err(LuaEngineError::Lua)?;
    rs.set("open_file", open_file)
        .map_err(LuaEngineError::Lua)?;
    ...
    let run_cmd = lua
        ...
        .map_err(LuaEngineError::Lua)?;
    rs.set("run_command", run_cmd)
        .map_err(LuaEngineError::Lua)?;

    // radialsan.mouse_click(button: string?, clicks: integer?)
    let mouse_click = lua
        .create_function(|_, (button, clicks): (Option<String>, Option<u64>)| {
        ...
        .map_err(LuaEngineError::Lua)?;
    rs.set("mouse_click", mouse_click)
        .map_err(LuaEngineError::Lua)?;

    // radialsan.clipboard(action: string) -- "copy", "cut", "paste"
    let clipboard = lua
        .create_function(|_, action: String| {
