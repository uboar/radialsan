...
<script lang="ts">
  ...
  export let menuOptions: Array<{ id: string; name: string }> | undefined =
    undefined;
  ...
  function handleSendKeyModifierToggle(modifier: ModifierName) {
    ...
    updateParam(
      "keys",
      buildKeyCombo(toggleModifier(combo.modifiers, modifier), combo.key),
    );
  }
...
<div class="space-y-3">
  <div class="min-w-0">
    <label
      ...
      for="action-type">{$t("editor.actionType")}</label
    >
    <select
      id="action-type"
      value={action.type}
  ...
  {#if action.type === "sendKey"}
    ...
    <div class="space-y-2">
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="send-key-key">{$t("actions.keys")}</label
        >
        <div class="flex min-w-0 flex-wrap items-center gap-2">
          ...
          <span
            class="rounded-lg border border-theme-border bg-theme-bg-tertiary px-3 py-2 font-mono text-sm leading-none text-theme-text-primary"
          >
      ...
      <div class="flex flex-wrap items-center gap-2">
        <span class="mr-1 shrink-0 text-xs text-theme-text-secondary"
          >{$t("actions.keyModifiers")}:</span
        >
  ...
  {#if action.type === "sendText"}
    <div class="min-w-0">
      <label
        class="block text-xs text-theme-text-secondary mb-1"
        for="send-text-value">{$t("actions.text")}</label
      >
      <input
        id="send-text-value"
        type="text"
  ...
  {#if action.type === "mouseClick"}
    <div class="space-y-2">
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="mouse-button">{$t("actions.button")}</label
        >
        <select
          id="mouse-button"
      ...
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="mouse-clicks">{$t("actions.clicks")}</label
        >
        <input
          id="mouse-clicks"
          type="number"
  ...
  {#if action.type === "openUrl"}
    <div>
      <label class="block text-xs text-theme-text-secondary mb-1" for="open-url"
        >{$t("actions.url")}</label
      >
      <input
        id="open-url"
  ...
  {#if action.type === "openFile" || action.type === "openFolder"}
    <div>
      <label
        class="block text-xs text-theme-text-secondary mb-1"
        for="open-path">{$t("actions.path")}</label
      >
      <input
        id="open-path"
        type="text"
  ...
  {#if action.type === "runCommand"}
    <div class="space-y-2">
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="run-command">{$t("actions.command")}</label
        >
        <input
          id="run-command"
          type="text"
  ...
  {#if action.type === "runScript"}
    <div class="space-y-2">
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="script-interpreter">{$t("actions.interpreter")}</label
        >
        <input
      ...
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="script-path">{$t("actions.scriptPath")}</label
        >
        <input
          id="script-path"
          type="text"
  ...
  {#if action.type === "clipboard"}
    <div class="min-w-0">
      <label
        class="block text-xs text-theme-text-secondary mb-1"
        for="clipboard-operation">{$t("actions.clipboardOperation")}</label
      >
      <select
        id="clipboard-operation"
        value={(params.operation as string) ||
  ...
  {#if action.type === "mediaControl"}
    <div class="min-w-0">
      <label
        class="block text-xs text-theme-text-secondary mb-1"
        for="media-action">{$t("actions.mediaAction")}</label
      >
      <select
        id="media-action"
  ...
  {#if action.type === "runLua"}
    <div class="space-y-2">
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="lua-script">{$t("actions.luaScript")}</label
        >
        <textarea
          id="lua-script"
  ...
  {#if action.type === "submenu" && menuOptions}
    <div class="space-y-2">
      <div>
        <label
          class="block text-xs text-theme-text-secondary mb-1"
          for="submenu-target">{$t("actions.targetMenu")}</label
        >
        <select
          id="submenu-target"
