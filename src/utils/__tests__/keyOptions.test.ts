...
import { buildKeyCombo, parseKeyCombo, SEND_KEY_OPTIONS, toggleModifier } from '../keyOptions';

describe('keyOptions', () => {
  it('parses and normalizes key combos', () => {
    expect(parseKeyCombo('shift+ctrl+a')).toEqual({ modifiers: ['Ctrl', 'Shift'], key: 'A' });
    expect(parseKeyCombo('meta+pagedown')).toEqual({ modifiers: ['Meta'], key: 'PageDown' });
