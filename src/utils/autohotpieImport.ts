  ...
  for (const ahpProfile of appProfiles) {
    ...
    for (const ahpPieKey of ahpProfile.pieKeys || []) {
      ...
      if (ahpMenus.length > 0) {
        ...
        const slices: Slice[] = (ahpMenu.functions || []).map(
          (fn: any, i: number) => ({
        ...
        menus.push({
          ...
          appearanceOverrides: {
            ...
            outerRadius: ahpMenu.radius
              ? ahpMenu.radius + (ahpMenu.thickness || 100)
              : undefined,
          },
          slices,
        });
...
function convertAhpFunction(fn: any): Action {
  ...
  switch (ahkFunc) {
    ...
    case "runScript":
      return {
        type: "runCommand",
        params: { command: params.scriptPath || "", shell: true },
      };
    case "openFolder":
    ...
    case "switchApplication":
      return {
        type: "runCommand",
        params: { command: params.windowTitle || "", shell: false },
      };
    default:
