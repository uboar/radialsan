...
const NAVIGATION_KEYS = ['Up', 'Down', 'Left', 'Right', 'Home', 'End', 'PageUp', 'PageDown'] as const;
...
export function parseKeyCombo(value: string): { modifiers: ModifierName[]; key: string } {
  const parts = value.split('+').map((part) => part.trim());
  const key = canonicalizeKeyName(parts[parts.length - 1] ?? '');
  const modifiers = MODIFIER_NAMES.filter((modifier) =>
    parts.slice(0, -1).filter(Boolean).some((part) => part.toLowerCase() === modifier.toLowerCase())
  );
  return { modifiers, key };
}

export function buildKeyCombo(modifiers: readonly string[], key: string): string {
  const sortedModifiers = MODIFIER_NAMES.filter((modifier) => modifiers.includes(modifier));
...
export function toggleModifier(modifiers: readonly ModifierName[], modifier: ModifierName): ModifierName[] {
  if (modifiers.includes(modifier)) {
    return modifiers.filter((existing) => existing !== modifier);
  }
  return MODIFIER_NAMES.filter((existing) => existing === modifier || modifiers.includes(existing));
}
